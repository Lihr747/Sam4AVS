...
def save_mask(pred_masks, save_base_path, video_name_list, vis_raw_img, raw_img_path, visual_heatmap):
    ...
    for idx in range(bs):
        ...
        for video_id in range(len(one_video_masks)):
            ...
            output_name = "%s.mp4_%d.png"%(video_name, video_id+1)
            output_vis = "%s.mp4_vis%d.png"%(video_name, video_id+1)
            ...
            if vis_raw_img:
                raw_img = os.path.join(raw_img_path, video_name, output_name)
