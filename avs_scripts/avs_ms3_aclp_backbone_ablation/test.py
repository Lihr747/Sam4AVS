...
if __name__ == "__main__":
    ...
    setup_logging(filename=os.path.join(log_path, 'text.txt'))
    ...
    session_name = os.path.basename(args.log_dir)
    timestamp_position = session_name.rfind('_')
    pt_name = session_name[:timestamp_position]

    ckpt_folder = os.path.join(args.log_dir, 'checkpoints')
    args.weights = os.path.join(ckpt_folder, pt_name + '_best.pth')
    ...
    with torch.no_grad():
        for n_iter, batch_data in enumerate(test_dataloader):
            ...
            if args.save_pred_mask:
                ...
                save_mask(output.squeeze(1), mask_save_path, video_name_list, vis_raw_img=True, raw_img_path=os.path.join(cfg.DATA.DIR_IMG, split))
