...
class Pred_endecoder(nn.Module):
    ...
    def __init__(self, channel=256, args=None, config=None, tpavi_stages=[], tpavi_vv_flag=False, tpavi_va_flag=True,
        ...
        visual_path, audio_path = self.audioclip_path.split(',')
        import collections
        visual_params = collections.OrderedDict()
        audio_params = collections.OrderedDict()
        visual_weights = torch.load(visual_path)
        if audio_path != "none":
            audio_weights = torch.load(audio_path)
        if "CLIP" in visual_path:
            for k, v in visual_weights.items():
                if k.startswith('visual'):
                    visual_params[k.replace('visual.', '')] = v
        elif "resnet" in visual_path:
            for k, v in visual_weights.items():
        ...
        if "AudioCLIP" in audio_path:
            for k, v in audio_weights.items():
                if k.startswith('audio'):
                    audio_params[k.replace('audio.', '')] = v
        elif "ESRNXFBSP" in audio_path:
            # load form ESResNeXt-fbsp
            for k, v in audio_weights.items():
                if k.startswith('fc.weight') or k.startswith('fc.bias'):
                    continue
                else:
                    audio_params[k.replace('audio.', '')] = v
        elif "none" in audio_path:
            pass
        self.audio_encoder.load_state_dict(audio_params, strict=False) # TODO: put the fc layer into training
