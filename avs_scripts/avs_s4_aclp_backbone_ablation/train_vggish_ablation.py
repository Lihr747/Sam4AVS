...
from dataloader import S4Dataset, S4Dataset_vggish
...
if __name__ == "__main__":
    ...
    parser.add_argument('--log_dir', default='../../avsbench_data/train_logs/aclp_s4_vggish_ablation_logs', type=str)
    ...
    parser.add_argument('--audioclip_path', default='../../pretrained_backbones/AudioCLIP-Full-Training.pt', type=str)
    ...
    elif (args.visual_backbone).lower() == "audioclip_realfpn_weights_ablation":
        from model import ACLP_AVSModel_real_Semantic_FPN_diff_init as AVSModel
    elif (args.visual_backbone).lower() == "audioclip_realfpn_vggish_ablation":
        from model import ACLP_AVSModel_real_Semantic_FPN_AudioCLIP_Vgg as AVSModel
    else:
    ...
    # Data
    train_dataset = S4Dataset_vggish('train')
    ...
    val_dataset = S4Dataset_vggish('val')
    ...
    if(args.freeze_audio_backbone):
        conflicts.append('module.audio_encoder')
    ...
    test_dataset = S4Dataset_vggish('test')
    ...
    with torch.no_grad():
        for n_iter, batch_data in enumerate(test_dataloader):
            imgs, audio, mask, category_list, video_name_list = batch_data # [bs, 5, 3, 224, 224], [bs, 5, 1, 96, 64], [bs, 5, 1, 224, 224]
            ...
            if not args.not_save_pred_mask:
                ...
                save_mask(output.squeeze(1), mask_save_path, category_list, video_name_list, vis_raw_img=True, raw_img_path=os.path.join(cfg.DATA.DIR_IMG, 'test'))
