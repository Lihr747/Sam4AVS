...
from utils.utility import logger, mask_iou, Eval_Fmeasure, save_mask_point
...
def show_point(img, points, labels):
    img = cv2.resize(img, (224, 224))
    vis = img.copy()
    vis = cv2.cvtColor(vis, cv2.COLOR_BGR2RGB)
    for point, label in zip(points, labels):
        if label == 1:
            cv2.circle(vis, (int(point[0]), int(point[1])), 2, (0, 255, 0), -1)
        else:
            cv2.circle(vis, (int(point[0]), int(point[1])), 2, (0, 0, 255), -1)
    vis = vis.astype('uint8')
    return vis
    ...
    with torch.no_grad():
        for n_iter, batch_data in enumerate(test_dataloader):
            ...
            for i in range(len(image_paths)):
                ...
                point_vis = show_point(image, points, labels)
                ...
                point_visualization.append(point_vis)
            ...
            save_mask_point(pred_masks, mask_save_path, category_list, video_name_list, vis_raw_img=True, raw_img_path=os.path.join(cfg.DATA.DIR_IMG, split), visual_heatmap=visual_heatmap, point_vis=point_visualization)
