...
def save_mask(pred_masks, save_base_path, category_list, video_name_list, vis_raw_img, raw_img_path, visual_heatmap):
    # pred_mask: [bs*5, 1, 224, 224]
    # print(f"=> {len(video_name_list)} videos in this batch")

    # img_base_path =  os.path.join(cfg.DATA.DIR_IMG, split, category_list, video_name_list)
    ...
    for idx in range(bs):
        ...
        one_video_vis = visual_heatmap[idx]
        for video_id in range(len(one_video_masks)):
            one_mask = one_video_masks[video_id]
            one_vis_img = one_video_vis[video_id]
            output_name = "%s_%d.png"%(video_name, video_id+1)
            output_vis = "%s_vis%d.png"%(video_name, video_id+1)
            im = Image.fromarray(one_mask).convert('P')
            im.save(os.path.join(mask_save_path, output_name), format='PNG')
            cv2.imwrite(os.path.join(mask_save_path, output_vis), one_vis_img)
            if vis_raw_img:
                raw_img = os.path.join(raw_img_path, category, video_name, output_name)
                raw_img = Image.open(raw_img)
                dst = Image.blend(raw_img, im.convert('RGB'), 0.5)
                dst.save(os.path.join(mask_save_path, 'mask_pred_'+output_name), format='PNG')
            
def save_mask_point(pred_masks, save_base_path, category_list, video_name_list, vis_raw_img, raw_img_path, visual_heatmap, point_vis):
    # pred_mask: [bs*5, 1, 224, 224]
    # print(f"=> {len(video_name_list)} videos in this batch")
    ...
    for idx in range(bs):
        ...
        one_video_masks = pred_masks[idx] # [5, 1, 224, 224]
        one_video_vis = visual_heatmap[idx]
        one_video_point = point_vis[idx]
        for video_id in range(len(one_video_masks)):
            one_mask = one_video_masks[video_id]
            one_vis_img = one_video_vis[video_id]
            one_point_vis_img = one_video_point[video_id]
            output_name = "%s_%d.png"%(video_name, video_id+1)
            output_vis = "%s_vis%d.png"%(video_name, video_id+1)
            output_point_vis = "%s_point_vis%d.png"%(video_name, video_id+1)
            im = Image.fromarray(one_mask).convert('P')
            im.save(os.path.join(mask_save_path, output_name), format='PNG')
            cv2.imwrite(os.path.join(mask_save_path, output_vis), one_vis_img)
            cv2.imwrite(os.path.join(mask_save_path, output_point_vis), one_point_vis_img)
            if vis_raw_img:
                raw_img = os.path.join(raw_img_path, category, video_name, output_name)
                raw_img = Image.open(raw_img)
                dst = Image.blend(raw_img, im.convert('RGB'), 0.5)
...
def save_mask_box(pred_masks, save_base_path, category_list, video_name_list, vis_raw_img, raw_img_path, visual_heatmap, box_vis):
    # pred_mask: [bs*5, 1, 224, 224]
    # print(f"=> {len(video_name_list)} videos in this batch")
    ...
    for idx in range(bs):
        ...
        one_video_masks = pred_masks[idx] # [5, 1, 224, 224]
        one_video_vis = visual_heatmap[idx]
        one_video_box = box_vis[idx]
        for video_id in range(len(one_video_masks)):
            one_mask = one_video_masks[video_id]
            one_vis_img = one_video_vis[video_id]
            one_box_vis_img = one_video_box[video_id]
            output_name = "%s_%d.png"%(video_name, video_id+1)
            output_vis = "%s_vis%d.png"%(video_name, video_id+1)
            output_box_vis = "%s_box_vis%d.png"%(video_name, video_id+1)
            im = Image.fromarray(one_mask).convert('P')
            im.save(os.path.join(mask_save_path, output_name), format='PNG')
            cv2.imwrite(os.path.join(mask_save_path, output_vis), one_vis_img)
            cv2.imwrite(os.path.join(mask_save_path, output_box_vis), one_box_vis_img)
            if vis_raw_img:
                raw_img = os.path.join(raw_img_path, category, video_name, output_name)
                raw_img = Image.open(raw_img)
                dst = Image.blend(raw_img, im.convert('RGB'), 0.5)
