...
def save_mask(pred_masks, gt_masks, save_base_path, category_list=None, video_name_list=None, vis_raw_img=False, raw_img_path=None):
    ...
    gt_masks = gt_masks.view(-1, 5, gt_masks.shape[-2], gt_masks.shape[-1])
    gt_masks = gt_masks.cpu().data.numpy().astype(np.uint8)
    gt_masks *= 255
    ...
    for idx in range(bs):
        ...
        one_video_gt_masks = gt_masks[idx]
        for video_id in range(len(one_video_masks)):
            one_mask = one_video_masks[video_id]
            one_gt_mask = one_video_gt_masks[video_id]
            output_name = "%s_%d.png"%(video_name, video_id+1)
            output_gt_name = "%s_%d_gt.png"%(video_name, video_id+1)
            im = Image.fromarray(one_mask).convert('P')
            im.save(os.path.join(mask_save_path, output_name), format='PNG')
            im_gt = Image.fromarray(one_gt_mask).convert('P')
            im_gt.save(os.path.join(mask_save_path, output_gt_name), format='PNG')
            if vis_raw_img:
